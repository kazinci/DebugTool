...
namespace FileMonitor
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
        ...
        string originalGiftsResults = @"C:\Documents and Settings\Administrator\Local Settings\Temp\GiftsResults.tmp";
        string pcName, partNumber, sn = String.Empty;
        ...
        private void Form1_Load(object sender, System.EventArgs e) //it will be loaded for first
        {
            logger("Program started!"); // program started
            try
            {
                File.Delete(@"c:\Documents and Settings\Administrator\Local Settings\Temp\GiftsResults.tmp");
                File.Delete(Path.Combine(Path.GetTempPath(), "GiftsResults.tmp"));
                File.Delete(@"C:\GiftsResults.tmp");
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
            }
            buildErrorFileArray(set.sourceDir1);// uploading Listbox1 with ERROR files
            buildErrorFileArray(set.sourceDir2);// uploading Listbox1 with ERROR files
            Rectangle monitorSize = Screen.PrimaryScreen.WorkingArea;// size of the screen
            monHeight = monitorSize.Height;
            monWidth = monitorSize.Width;
            pcName = Convert.ToString(Registry.GetValue(@"HKEY_CURRENT_USER\Software\Gilat Satellite Networks Ltd.\SEII_JTAG\Settings", "Setup Stand", ""));
            this.WindowState = FormWindowState.Minimized; // minimize to tray
            startCycle();
        }

        private void timer1_Tick(object sender, System.EventArgs e)//it will run in every 5sec (by default) 
        {
            statusBar1.Text = "Cycle started!";            

            if (File.Exists(originalGiftsResults))//Testing started!
            {
                monitorGiftsTemp();//continously checking the existance of the GiftsTemp in Temp folder
            }
            else
            {
                //reset, now we listening again for the GiftsTemp file in Temp folder
                if (logfileCopied)
        ...
        private void monitorGiftsTemp() //periodically check GiftsResults.tmp if it contains ERRORS, this will run autoClickAndCopy() and monitorErrFiles()
        {
            string copiedGiftsResults = @"C:\GiftsResults.tmp";

            try
            {
                File.Copy(originalGiftsResults, copiedGiftsResults, true);//must be copied periodically, because the original file is used by GIFTS
            }
            catch (Exception except)
            {
                //logger("GiftsResults.tmp not found!");
                return;
            }            

            if (File.Exists(copiedGiftsResults) && logfileCopied == false)
            {
                using (FileStream fs = new FileStream(copiedGiftsResults, FileMode.Open))
                {
                    using (StreamReader sr = new StreamReader(fs))
                    {
                        ...
                        while ((logRow = sr.ReadLine()) != null)
                        {
                            if (logRow == "Result=0")
                            {
                                ...
                                //Get the serial and partnumber from the registry
                                sn = Convert.ToString(Registry.GetValue(@"HKEY_CURRENT_USER\Software\Gilat Satellite Networks Ltd.\SEII_JTAG\Serials", "Serial Number 1", 0));
                                partNumber = Convert.ToString(Registry.GetValue(@"HKEY_CURRENT_USER\Software\Gilat Satellite Networks Ltd.\SEII_JTAG\Serials", "Part_number 1", ""));

                                autoClickAndCopy(); //saving LOG file to the server
                                checkErrFiles(); // check all  ERROR files if they were modified
                                logfileCopied = true;   //Log file copied
                            }
                        }
                    }
                }
        ...
        private void autoClickAndCopy()
        {
            ...
            if (set.autoClickAndCopy)//maybe disabled in Options
            {
                FreezeMouse();
                AutoItX3Declarations.AU3_MouseClick("left", monWidth - 48, monHeight - 69, 1, 1); // Click on "Show log" button
                AutoItX3Declarations.AU3_Sleep(500); // wait

                AutoItX3Declarations.AU3_Send("!{f}", 0); // Alt+f = file menu                
                //AutoItX3Declarations.AU3_Sleep(10); 

                AutoItX3Declarations.AU3_Send("{ENTER}", 0); // Save menupoint
                //AutoItX3Declarations.AU3_Sleep(10);

                AutoItX3Declarations.AU3_Send(sn + ".log", 0); // file name
                //AutoItX3Declarations.AU3_Sleep(10);

                for (int i = 0; i < 6; ++i)
                {
                    AutoItX3Declarations.AU3_Send("{TAB}", 0);// TAB x 6
                    //AutoItX3Declarations.AU3_Sleep(10);
                }
                AutoItX3Declarations.AU3_Send("{DOWN}", 0);// Down arrow
                //AutoItX3Declarations.AU3_Sleep(10);

                AutoItX3Declarations.AU3_Send("{DOWN}", 0);//  Down arrow
                //AutoItX3Declarations.AU3_Sleep(10);

                AutoItX3Declarations.AU3_Send("{ENTER}", 0); // Documents folder
                //AutoItX3Declarations.AU3_Sleep(10);

                AutoItX3Declarations.AU3_Send("{TAB}", 0); // move next
                //AutoItX3Declarations.AU3_Sleep(10);

                AutoItX3Declarations.AU3_Send("{TAB}", 0); // move next
                //AutoItX3Declarations.AU3_Sleep(10);

                AutoItX3Declarations.AU3_Send("!{s}", 0); // save
                //AutoItX3Declarations.AU3_Sleep(10);

                AutoItX3Declarations.AU3_Send("!{F4}", 0);//Close log window
                ThawMouse();
                Cursor.Position = pos;// returning to the last position
                ...
                try
                {
                    ...
                    if (File.Exists(from))
                    {
                        //jumping to the last LOG section
                        ...
                        File.Delete(from); //we can delete it because it's already in the memory
                        ...
                        while (File.Exists(to)) // if we already have a file with the same name, than rename it
                        {
                            ++fileCounter;
                    ...
                    {
                        Console.WriteLine("{0}: Waiting 1 sec..",cnt);
                        AutoItX3Declarations.AU3_Sleep(1000);//waiting 1sec, because it's possible that the LOG file will be more then 5-10Mb
                        ++cnt;
                        goto START;
                    }
                }
        ...
        private void checkErrFiles() // check for file change in ERROR files and if modified, copy to the server
        {
            int index = 0;
            ...
            {
                modifiedTime = File.GetLastWriteTime(f).ToString();

                if (modifiedTime != listBox2.Items[index].ToString())// if it's modified than copy it
                {
                    statusBar1.Text = "File changed!!!";

                    listBox2.Items.RemoveAt(index); //update modified time in the array
                    listBox2.Items.Insert(index, modifiedTime);                    
                    ...
                    //Search for "ERRORS 1" word inside the file, if it's not found, than we will skip the copying 
                    string row = String.Empty;
                    using (StreamReader sr = new StreamReader(Path.Combine(sourceDir, fName + ext)))
                    {
                        while ((row = sr.ReadLine()) != null)
                        {
                            if (row.IndexOf("ERRORS") == 0)
                            {
                                string[] splitError = row.Split(new Char[] { ' ' });
                                if (int.Parse(splitError[1]) > 0)//if there is more than one error
                                {
                                    //
                                    ...
                                    strB.Append(sn);
                                    strB.Append("_");
                                    strB.Append(fileCounter);
                                    strB.Append("-");


                                    strB.Append(fName);//original filename
                                    strB.Append(ext);//.err
                                    ...
                                    try
                                    {
                                        //while (File.Exists(finalBackupFile))// if it's already exists (sn_x0-filename.err) rename by _x1, _x2, etc...
                                        //{
                                        //    ++fileCounter;
                                        //    string[] split = finalBackupFile.Split(new char[] { '_' });

                                        //    for (int k = 0; k < split.Length - 1; ++k)// (sn_)
                                        //    {
                                        //        strB.Append(split[k]);
                                        //        strB.Append("_");
                                        //    }
                                        //    strB.Append(fileCounter);//(sn_1)
                                        //    strB.Append("-");//(sn_1-)
                                        //    strB.Append(fName);//(sn_1-filename)
                                        //    strB.Append(ext);//(sn_1-filename.err)
                                        //    finalBackupFile = strB.ToString();// saving new filename
                                        //}
                                        //fileCounter = 0;
                                    ...
                                    catch (Exception ex)
                                    {
                                        ...
                                        logger("Copy error!"+ ex.Message);
                                        return;
                                    }
                                    logger(Path.GetFullPath(f) + " copied to " + finalBackupFile);
                                }
                            }
                        }
                    };// END Streamreader
        ...
        private void buildErrorFileArray(string sourceDir)//find every error file and save it in the array (loaded at initialization)
        {
            try
            {
        ...
        private void copyAPLBtn_Click(object sender, System.EventArgs e)//copy every APL with directories to the server
        {
            foreach (string errFile in listBox1.Items)
            {
                string sourceDir = Path.GetDirectoryName(errFile) + "\\";
                string backupFolderName = String.Empty;
                string[] aplList = Directory.GetFiles(sourceDir, "*.apl"); //maybe here is more than one APL file               
                string[] splitDir = sourceDir.Split(new Char[] { ':' });
                for (int q = 1; q < splitDir.Length; q++)
                {
                    backupFolderName += splitDir[q];
                }
                string finalBackupDir = set.backupFolder + backupFolderName + "\\APL\\";
                foreach (string aplFile in aplList)
                {
                    string aplFileName = Path.GetFileName(aplFile);
                    Directory.CreateDirectory(finalBackupDir);
                    File.Copy(Path.Combine(sourceDir, aplFileName), Path.Combine(finalBackupDir, aplFileName), true);
                }
            }
        }

        public void logger(string text) // print out info's to "C:\filemonitor.log"
        {
            Console.WriteLine(text);
            if (set.useLog)
                using (StreamWriter log = new StreamWriter(@"C:\debug_tool.log", true))
                {
        ...
        internal void startCycle ()
        {
            ...
            this.timer1.Interval = set.checkInterval * 1000;
